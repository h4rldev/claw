...
int write_config(Config *config) {
  ...
  char *listen_path = config->listen_path;
  json_object_set_new(root, "listen_path", json_string(listen_path));
...
int read_config(Config *config) {
  ...
  json_t *listen_path_string = json_object_get(root, "listen_path");

  char *listen_path = (char *)malloc(1024);
  if (!listen_path) {
    json_decref(root);
    return -1;
  }

  if (json_is_string(listen_path_string)) {
    strlcpy(listen_path, json_string_value(listen_path_string), 1024);
  } else {
    json_decref(root);
    free(listen_path);

    return handle_parse_err("root", "listen_path");
  }
