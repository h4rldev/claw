...
static int create_listener(char *ip, unsigned int port) {
  ...
  if ((return_code = uv_tcp_bind(&listener, (struct sockaddr *)&addr, 0)) !=
      0) {
    fprintf(stderr, "uv_tcp_bind:%s\n", uv_strerror(return_code));
    goto Error;
  }
  if ((return_code = uv_listen((uv_stream_t *)&listener, 128, on_accept)) !=
      0) {
...
static int index_handler(h2o_handler_t *self, h2o_req_t *req) {
  ...
  h2o_iovec_t body =
      h2o_strdup(&req->pool, H2O_STRLIT("<h1>claw, a webhook listener</h1>\n"
...
int main(int argc, char **argv) {
  ...
  pathconf = register_handler(hostconf, "/", index_handler);
  if (logfh != NULL)
    h2o_access_log_register(pathconf, logfh);
  if (log2file != NULL)
    h2o_access_log_register(pathconf, log2file);
...
NotCompress:
  ...
  printf("claw: running on 127.0.0.1:%u\n", server_config.network.port);
  free_config(&server_config);
